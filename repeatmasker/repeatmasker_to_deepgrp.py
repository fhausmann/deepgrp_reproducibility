...
_COMP_TBL: Dict[int, str] = str.maketrans({"A": "T", "T": "A", "C": "G", "G": "C"})
_BASES: str = "ACGT"
_MOTIF0: str = "GGAAT"
...
_TYPES: collections.defaultdict = collections.defaultdict(
    int, **{k: v for v, k in enumerate(_REPEATS, 1)}
)
...
class Repeat:  # pylint: disable=too-few-public-methods
    ...
    def __str__(self) -> str:
        return "{}\t{}\t{}\t{}\t{}\t{}".format(
            self.ctg, self.start, self.end, self.typ, self.rep, self.fam
        )
...
def _check_motifs(
    motif: str, motif_mut_hash: Dict[str, int], motif_hash: Dict[str, int]
) -> Tuple[int, int]:
    count = 0
    count_mut = 0
...
def read_repeatmasker(
    motif_hash: Dict[str, int], motif_mut_hash: Dict[str, int], filestream: TextIO
) -> Iterator[Repeat]:
    ...
    for line in filestream.readlines():
        ...
        if repeat.typ == 0 and repeat.fam in ("Simple_repeat", "Satellite"):
            ...
            if motif is not None:
                motif: str = motif.group(1)
                if motif in motif_hash:
                    repeat.typ = _TYPES["HSATII"]
                elif len(motif) % len0 == 0:
                    count, count_mut = _check_motifs(motif, motif_mut_hash, motif_hash)
...
def main():
    ...
    parser.add_argument("--outfile", type=str, default="repeats.tsv")
    args = parser.parse_args()
    with pathlib.Path(args.outfile).open("w") as output:
        for filename in args.infiles:
            filename = pathlib.Path(filename)
            with filename.open("r") as file:
                for repeat in read_repeatmasker(MOTIF_HASH, MOTIF_MUT_HASH, file):
                    output.write(str(repeat) + "\n")
